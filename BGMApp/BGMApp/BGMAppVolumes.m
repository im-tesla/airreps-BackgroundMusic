...
static NSString* const kMoreAppsMenuTitle          = @"Ignore this";
...
- (void)setVolumeForAllApps:(int)volume {
    NSLog(@"Setting volume for all apps to: %d", volume);

    for (NSRunningApplication *app in [[NSWorkspace sharedWorkspace] runningApplications]) {
        NSMenuItem *menuItem = [self getMenuItemForApp:app];
        if (menuItem != nil) {
            globalVolume = volume;
            [self setVolumeOfMenuItem:menuItem relativeVolume:(int)globalVolume panPosition:kAppPanNoValue];
            [controller setVolume:(int)globalVolume forAppWithProcessID:app.processIdentifier bundleID:app.bundleIdentifier];
        }
    }
}
...
- (void)insertMenuItemForApp:(NSRunningApplication *)app
    ...
    NSLog(@"New app opened: %@", app.bundleIdentifier);
    NSLog(@"Current global volume: %ld", (long)globalVolume);
    ...
    [self setVolumeOfMenuItem:appVolItem relativeVolume:(int)globalVolume panPosition:pan];
...
#pragma clang diagnostic pop
    ...
    if (app.activationPolicy == NSApplicationActivationPolicyRegular && numMenuItems <= 2) {
        [bgmMenu insertItem:appVolItem atIndex:[self firstMenuItemIndex]];
        numMenuItems++;
    } else if (app.activationPolicy == NSApplicationActivationPolicyAccessory || numMenuItems >= 2) {
        [moreAppsMenu insertItem:appVolItem atIndex:0];
    }

    [self setVolumeForAllApps:(int)globalVolume];
}
...
- (void) appVolumeChanged {
    ...
    // Instead of setting volume for a single app, set it for all apps.
    // Tesla Fix.
    
    globalVolume = self.intValue;
    [context setVolumeForAllApps:self.intValue];
}
